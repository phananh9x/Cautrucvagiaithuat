...
void NhapSinhVien(){
	VeEditText(30,5,"    Nhap Ma SV     ");
	VeEditText(27,8,"       Nhap Ho SV        ");
	VeEditText(27,11,"       Nhap Ten SV       ");
	VeEditText(30,14,"    Nhap GT SV     ");
	}
void NhapLop(){
	setcolor(27,16);
	VeEditText(30,5,"    Nhap Ma Lop    ");
	VeEditText(22,8,"            Nhap Ten Lop           ");
	}
void NhapMonHoc(){
	VeEditText(30,5,"    Nhap Ma MH     ");
	VeEditText(22,8,"            Nhap Ten MH            ");
	VeEditText(30,11,"     Nhap STCLT    ");
	VeEditText(30,14,"     Nhap STCTH    ");
	}
	
void NhapDiem(){
	VeEditText(30,5,"    Nhap Ma SV     ");
	VeEditText(30,8,"    Nhap Ma MH     ");
	VeEditText(30,11,"   Nhap Lan Thi    ");
	VeEditText(30,14,"    Nhap Diem      ");
	}
...
void BangLOP ( int maxTrang ,int x ,int y)
{
	...
	for(int i=0;i<5;i++)  cout<<(char)196;   cout<<(char)194 ;
	for(int i=0;i<13;i++) cout<<(char)196;  cout<<(char)194 ;
	for(int i=0;i<31;i++) cout<<(char)196;   cout<<(char)191;

	gotoxy(x,y+1); cout<<(char)179<<" STT "<<(char)179<<"    MA LOP   "<<(char)179<<"            TEN LOP            "<<(char)179;

	gotoxy(x,y+2); cout<<(char)195;
	for(int i=0;i<5;i++)  cout<<(char)196;   cout<<(char)197 ;
	for(int i=0;i<13;i++) cout<<(char)196;  cout<<(char)197 ;
	for(int i=0;i<31;i++) cout<<(char)196;   cout<<(char)180;

	gotoxy(x,y+3+maxTrang); cout<<(char)192;
	for(int i=0;i<5;i++)  cout<<(char)196;   cout<<(char)193 ;
	for(int i=0;i<13;i++) cout<<(char)196;   cout<<(char)193 ;
	for(int i=0;i<31;i++) cout<<(char)196;   cout<<(char)217 ;
...
void BangSV ( int maxTrang ,int x ,int y)
  {
	...
	gotoxy(x,y+1); cout<<(char)179<<" STT "<<(char)179<<"    MA SV    "<<(char)179<<"     HO SV      "<<(char)179<<"      TEN SV      "<<(char)179<<" GIOI TINH SV "<<(char)179;
		...
		strcpy(mh.MAMH,MAMH);
		 NHAPTENMH:	        
	        ...
	        while(strlen(mh.TENMH)==0&&(c==KEY_ENTER||c==KEY_DOWN)){
	        	ThongBao("Ban Chua Nhap Ten Mon Hoc !!",16,28,25,40);
	        	goto NHAPTENMH;
	        }
	        if(c==KEY_UP)
			{
...
NHAPSTCLTMH:        
	        ...
	        while(strlen(stclt)==0&&(c==KEY_ENTER||c==KEY_DOWN)){
	        	ThongBao("Ban Chua Nhap STCLT Mon Hoc !!",16,28,25,40);
	        	goto NHAPSTCLTMH;
	        }
	        if(c==KEY_UP)
			{
   ...
   NHAPSTCTHMH:	        
	        ...
	        while(strlen(stcth) ==0&&c==KEY_ENTER){
	        	ThongBao("Ban Chua Nhap STCTH Mon Hoc !!",16,28,25,40);
	        	goto NHAPSTCTHMH;
	        }
	    mh.STCLT=atoi(stclt);
		mh.STCTH=atoi(stcth);       
...
//*******Tim kiem mon hoc**************////////////	
void Timkiem_MH(list_MH &listMH,int vitri,char *mamh){
		gotoxy(5, 43); cout<< ( 1 );
		gotoxy(13,43); cout<<listMH.mh[vitri].MAMH ;
		gotoxy(28,43); cout<<listMH.mh[vitri].TENMH ;
		gotoxy(58,43); cout<<listMH.mh[vitri].STCLT ;
		gotoxy(69,43); cout<<listMH.mh[vitri].STCTH ;		
	}
...
int Sua_SV(list_LOP &listLOP ,char *MASV){
	...
	int vttensv=0,vthosv=0,vtgioitinh=0;
	char c;
	SINHVIEN sv;
	if(vitri!=-1){
	for(PTRSV p =listLOP.lop[vitri].contro ;p!=NULL;p=p->next)
	{
		if(strcmp(p->sv.MASV,MASV)==0)
		{
			...
			strcpy(sv.MASV,MASV);
    NHAPHOSV:       
		    c=HamNhapDuLieu2(29,9,vthosv,20,sv.HO,1);
	        if(c==KEY_ESC)
			{
	        	return 2;
	        }
			...
			{
	        	ThongBao("Ban Chua Nhap Ho SV !!",16,28,25,36);
	        	goto NHAPHOSV;
	        }
	        if(c==KEY_DOWN)
			{
	...
	NHAPTENSV:        
		    c=HamNhapDuLieu2(29,12,vttensv,20,sv.TEN,1);
	        if(c==KEY_ESC)
			{
	        	return 2;
	        }
			...
			{
	        	ThongBao("Ban Chua Nhap Ten SV !!",16,28,25,36);
	        	goto NHAPTENSV;
	        }
	        if(c==KEY_DOWN)
			{
	...
	NHAPGTSV:      
			...
			{
	        	ThongBao("Ban Chua Nhap Gioi Tinh SV !!",16,28,25,36);
	        	goto NHAPGTSV;
	        }
	        if(c==KEY_UP)
			{
...
//*******Luu danh sach diem**************////////////
int LuuDS_DIEM(PTRDIEM listDIEM,char *tenfile){
	fstream file;
	DIEM diem;
	file.open(tenfile,ios::out|ios::binary|ios::trunc);
	if(!file) 
...
void LayDS_DIEM(PTRDIEM &listDIEM,char *tenfile){
	fstream file;
	file.open(tenfile,ios::in|ios::binary);
	if(file.fail()) { file.open(tenfile,ios::out|ios::binary); return; };
	if(!file){cout<<"\n Khong the mo file "<<tenfile; return;};
...
XUATDS:	
	...
	if(listLOP.n < 0)
	{
		ThongBao("Chua co lop trong danh sach !",16,28,25,40);
	}
	...
	switch(key)
	{
		...
		case KEY_F1:
			...
			NhapLop();
  NHAPMALOP:      
		    c=HamNhapDuLieu2(32,6,vtmalop,10,lop.MALOP,0);
	        if(c==KEY_ESC)
			{
	        	goto XUATDS;
	        }
			...
			{
	        	ThongBao("Ban Chua Nhap Ma Lop !!",16,28,25,40);
	        	goto NHAPMALOP;
	        }
	        while(KiemTra_Lop(listLOP,lop.MALOP)!=-1)
			{
				ThongBao("Ma Lop Da Ton Tai !!",16,28,25,40);
	        	goto NHAPMALOP;
	        }
	        if(c==KEY_UP)
			{
   ...
   NHAPTENLOP:	        
			...
			{
	        	ThongBao("Ban Chua Nhap Ten Lop !!",16,28,25,40);
	        	goto NHAPTENLOP;
	        }
	        if(c==KEY_UP)
			{
			...
			{
	        	ThongBao("Them Lop Hoc Thanh cong !!",16,28,25,40);
	        }
			else
			{
	        	ThongBao("Het Bo Nho Lop !!",16,28,25,40);	
	        }
			break;	
		case KEY_F2:
			VeNutChucNang(1,MENUCHUCNANG,15,16);
			VeEditText(30,5,"    Nhap Ma Lop   ");
			c=HamNhapDuLieu(32,6,10,lop.MALOP,0);
	        if(c==KEY_ESC)
			{
	        	goto XUATDS;
	        }
	        ...
	        ThongBao("Sua Thanh Cong Lop !!",16,28,25,40);
			}
			else
			{
				ThongBao("Khong Tim Thay Ma Lop !!",16,28,25,40);		
			}
			break;	
		case KEY_F3:
			...
			if(vitri!=-1)
			{
				int kq=HopThoaiYESNO("Ban Co Muon Xoa Lop ?",COKHONG,24,7,3,15,15,15);
				if(kq==1)
				{
					int n=Xoa_LOP(listLOP,vitri);
					if(n==1)
					{
					   ThongBao("Xoa Thanh Cong Lop !!",16,28,25,40);
					}
					else
					if(n==0)
					{
						ThongBao("Lop Co Sinh Vien Khong Xoa Duoc !!",16,28,25,40);
					}
					else
					{
						ThongBao("Xoa That Bai Lop !!",16,28,25,40);
					}
					}
			}
			else
		...
		case KEY_ESC:
			...
			int kq=HopThoaiYESNO("Ban co muon Luu Du Lieu Vao File ?",COKHONG,19,7,3,15,15,15);
			if(kq==1) 
			{
				if(Luu_LOP(listLOP,tenfilelop)==1)
				{
					ThongBao("Luu File Thanh Cong !!",16,28,25,40);	
				}
				else
				{
				   ThongBao("Luu File That Bai !!",16,28,25,40);		
				}
			}
			if(kq==1||kq==0)
			return;
...
void HienThiGoiYLop(list_LOP listLOP ,int background,int textcolor,int x,int y){
    ...
    if(listLOP.n ==0)
	{
		ThongBao("Chua co lop trong danh sach !",16,28,25,15);
	}
	else
	{
	if(listLOP.n < 15){
...
void thaotacmenuSV(list_LOP &listLOP,PTRDIEM &listDIEM)
{
	...
	int tongtrang,trang=0;
	int vitri=-1;
	int vtmasv=0,vttensv=0,vthosv=0,vtgioitinh=0;
	char menu[][30]={"ESC","F5"};
	int x=20;
	int y=18;
	int kq=0;
	char masv[30],malop[30];
	int stclt,stcth;
	char COKHONG[][10]={"CO","KHONG"};
	char key,c;
	char tenfile[]="DIEM.txt";
	LayDS_DIEM(listDIEM,tenfile);
	char tenfilelop[]="LOP.txt";
	char tenfilesv[]="SINHVIEN.txt";
	listLOP.n=0;
	LayDS_LOP(listLOP,tenfilelop);
    ...
    switch(c)
	 {
	  	...
	  	case KEY_ENTER:
	  		...
	  		if(vitri==-1)
			  {
	  			ThongBao("Khong Tim Thay Ma Lop !",16,28,25,15);
	  			goto NHAPMALOP;
	  		  }
		    break;		
	  }	
...
XUATDS:	
	...
	if(listLOP.lop[vitri].contro ==NULL)
	{
		ThongBao("Chua Sinh Vien Trong Lop Nay !",16,28,25,38);
	}
	...
	NHAPMASV:   
			...
			{
	        	ThongBao("Ban Chua Nhap Ma SV !!",16,28,25,38);
	        	c=HamNhapDuLieu2(32,6,vtmasv,10,sv.MASV,0);
	        }
	        while(Kiemtra_SV(listLOP,sv.MASV)!=NULL)
			{
				ThongBao("Ma SV da ton tai !!",16,28,25,38);
	        	goto NHAPMASV;
	        }
	        if(c==KEY_UP)
			{
	...
	NHAPHOSV:       
	        ...
	        while(strlen(sv.HO)==0&&(c==KEY_ENTER||c==KEY_DOWN))
			{
	        	ThongBao("Ban Chua Nhap Ho SV !!",16,28,25,38);
	        	goto NHAPHOSV;
	        }
	        if(c==KEY_DOWN)
			{
	...
	NHAPTENSV:        
	        ...
	        while(strlen(sv.TEN)==0&&(c==KEY_ENTER||c==KEY_DOWN))
			{
	        	ThongBao("Ban Chua Nhap Ten SV !!",16,28,25,38);
	        	goto NHAPTENSV;
	        }
	        if(c==KEY_DOWN)
			{
	...
	NHAPGTSV:      
	        ...
	        while(strlen(sv.PHAI)==0&&c==KEY_ENTER)
			{
	        	ThongBao("Ban Chua Nhap Gioi Tinh SV !!",16,28,25,38);
	        	goto NHAPGTSV;
	        }
	        if(c==KEY_UP)
			{
			...
			{
	        	ThongBao("Them Sinh Vien Thanh cong !!",16,28,25,38);
	        }
			else
			{
	        	ThongBao("Het Bo Nho Lop !!",16,28,25,38);	
	        }
			break;	
		case KEY_F2:
			vtmasv=0;
			...
			if(kq==1)
			{
	            ThongBao("Sua Thanh Cong Lop !!",16,28,25,38);
			}
			if(kq==0)
			{
				ThongBao("Khong Tim Thay SV !!",16,28,25,40);		
			}
			break;	
		case KEY_F3:
			vtmasv=0;
			...
			if(Kiemtra_SV(listLOP,masv)!=NULL){
				 kq=HopThoaiYESNO("Ban Co Muon Xoa SV ?",COKHONG,24,7,3,15,15,15);
				if(kq==1)
				{
					...
					if(n==1)
					{
					   ...
					   ThongBao("Xoa Thanh Cong SV !!",16,28,25,38);
					}
					else
					{
						ThongBao("Xoa That Bai SV !!",16,28,25,38);
					}
					}
			}
			else
			{
				ThongBao("Khong Tim Thay Ma Lop !!",16,28,25,38);		
			}
			break;	
		...
		case KEY_ESC:
			...
			if(kq==1) 
			{
				...
				{
					LuuDS_DIEM(listDIEM,tenfile);
					ThongBao("Luu File Thanh Cong !!",16,28,25,38);	
				}
				else
				{
				   ThongBao("Luu File That Bai !!",16,28,25,38);		
				}
			}
			if(kq==1||kq==0)
			return;
...
void thaotacmenuMH(list_MH &listMH,PTRDIEM &listDIEM){
	int tongtrang,trang=0;
	int vitri=-1;
...
XUATDS:	
	...
	for(int i=trang*15;i<15+15*trang&&i<listMH.n;i++)
	{
		gotoxy(5, 9+dong); cout<< ( i +1 );
		gotoxy(13,9+dong); cout<<listMH.mh[i].MAMH ;
		gotoxy(28,9+dong); cout<<listMH.mh[i].TENMH ;
		gotoxy(58,9+dong); cout<<listMH.mh[i].STCLT ;
		gotoxy(69,9+dong); cout<<listMH.mh[i].STCTH ;
		dong++;
	}
	if(listMH.n ==0)
	{
		ThongBao("Chua co mon hoc trong danh sach !",16,28,25,35);
	}
	MenuChucNang(MENUCHUCNANG,MENUCHUCNANGCHU,28,15,15,2,15,6,1);
	do{
		key=getch();
	}while(key!=KEY_F1&&key!=KEY_F2&&key!=KEY_F3&&key!=KEY_F4&&key!=KEY_ESC);
	...
	NHAPMAMH:   
			...
			{
	        	ThongBao("Ban Chua Nhap Ma MH !!",16,28,25,38);
	        	c=HamNhapDuLieu2(32,6,vtmamh,10,mh.MAMH,0);
	        }
	        while(KiemtraMAMH(listMH,mh.MAMH)!=-1)
			{
				ThongBao("Ma MH Da Ton Tai !!",16,28,25,38);
	        	goto NHAPMAMH;
	        }
	        if(c==KEY_UP)
			{
...
NHAPSTCLTMH:        
   ...
   NHAPSTCTHMH:	        
	        ...
	        mh.STCLT=atoi(stclt);
	        mh.STCTH=atoi(stcth);
	        if(Them_MH(listMH,mh,listMH.n+1)==1){
	        	ThongBao("Them Mon Hoc Thanh cong !!",16,28,25,40);
	        }else{
	        	ThongBao("Het Bo Nho Mon Hoc !!",16,28,25,40);	
	        	}
			break;	
		case KEY_F2:
			VeNutChucNang(1,MENUCHUCNANG,15,16);
	        ...
	        while(strlen(mamh) ==0&&c==KEY_ENTER){
	        	ThongBao("Ban Chua Nhap Ma Mon Hoc !!",16,28,25,40);
	        	c=HamNhapDuLieu(32,6,10,mamh,0);
	        }
			vitri=KiemtraMAMH(listMH,mamh);
			if(vitri!=-1)
			{
				VeEditText(22,8,"            Nhap Ten MH            ");
            	VeEditText(30,11,"     Nhap STCLT    ");
             	VeEditText(30,14,"     Nhap STCTH    ");
				if(Sua_MH(listMH,vitri,mamh)==1)
				{
	                ThongBao("Sua Thanh Cong Mon Hoc !!",16,28,25,40);
	            }else
				{
	            	goto XUATDS;
	            }
			...
			{
				ThongBao("Khong Tim Thay Ma Mon Hoc !!",16,28,25,40);		
			}
			break;	
		case KEY_F3:
			VeNutChucNang(2,MENUCHUCNANG,15,16);
			...
			vitri=KiemtraMAMH(listMH,mamh);
			if(vitri!=-1){
				int kq=HopThoaiYESNO("Ban Co Muon Xoa Mon Hoc ?",COKHONG,25,5,16,15,15,15);
				if(kq==1){
					if(Xoa_MH(listMH,vitri)==1){
					   if(listDIEM!=NULL)
					   {
					      Xoa_MH_DIEM(listDIEM,mamh);
					   }
					   ThongBao("Xoa Thanh Cong Mon Hoc !!",16,28,25,40);
					}else{
						ThongBao("Xoa That Bai Mon Hoc !!",16,28,25,40);
					}
					}
			}
			else
...
void HienThiGoiYSV(list_LOP listLOP ,int background,int textcolor,int x,int y){
	...
	if(soluong == 0)
	{
		ThongBao("Chua sinh vien nao het !",16,28,25,15);
	}
	else
	{
	if(soluong < 15){
...
void HienThiGoiYMH(list_MH listMH ,int background,int textcolor,int x,int y){
	int tongtrang,trang=0,soluong=0;
	char key;
    if(listMH.n ==0)
	{
		ThongBao("Chua co mon hoc trong danh sach !",16,28,25,15);
	}
	else
	{
...
XUAT:
	...
	{
		gotoxy(x+3, y+4+dong); cout<< ( i+1 );
		gotoxy(x+8,y+4+dong); cout<<listMH.mh[i].MAMH ;
		gotoxy(x+24,y+4+dong); cout<<listMH.mh[i].TENMH ;
		gotoxy(x+57,y+4+dong); cout<<listMH.mh[i].STCLT ;
		gotoxy(x+68,y+4+dong); cout<<listMH.mh[i].STCTH ;
		dong++;
	}
...
int CapNhatDanhSachDiem(PTRDIEM &listDIEM,char * MAMH,int soluong,char lan1[][4],char lan2[][4],SINHVIEN sv[],char *tenfile){
	int check=0;
	...
	for(int i=0;i< soluong;i++)
	{
		...
		if(lan1[i][0]!='\0')
		{
		  ...
		  for(PTRDIEM p=listDIEM;p!=NULL;p=p->next)
		  {
			if(strcmp(diem1.MASV,p->diem.MASV)==0&&strcmp(MAMH,p->diem.MAMH)==0&&p->diem.LAN==1)
			{
				p->diem.DIEMSV=diem1.DIEMSV;
				check=1;
			}
		  }
		  if(check==0){
	   ...
	   if(lan2[i][0]!='\0')
		{
		  ...
		  for(PTRDIEM p=listDIEM;p!=NULL;p=p->next)
		  {
			if(strcmp(diem2.MASV,p->diem.MASV)==0&&strcmp(MAMH,p->diem.MAMH)==0&&p->diem.LAN==2)
			{
				p->diem.DIEMSV=diem2.DIEMSV;
				check=1;
			}
		  }
		  if(check==0){
	...
	int kq= LuuDS_DIEM(listDIEM,tenfile);
	return kq;
}
int KiemTraNhapDiem(int soluong,char lan1[][4],char lan2[][4],int &vitri,int &toadoY,int &trang,int &loidiem1,int &loidiem2)
{
	...
	for(int i=0;i<soluong;i++)
	{
		...
		{
			ThongBao("Ban Chua Nhap Diem Thi Lan 1",16,28,25,38);
			vitri=i;
			toadoY =dong;
			trang=vitritrang;
			loidiem1=1;
	...
	for(int i=0;i<soluong;i++)
	{
		...
		{
			ThongBao("Ban Nhap Sai Diem Thi Lan 1 (0 <= Diem <= 10)",16,28,25,38);
			vitri=i;
			toadoY =dong;
			trang=vitritrang;
			loidiem1=1;
		...
		{
			ThongBao("Ban Nhap Sai Diem Thi Lan 2 (0 <= Diem <= 10)",16,28,25,38);
			vitri=i;
			toadoY =dong;
			trang=vitritrang;
			loidiem2=1;
...
//*******Thao tac menu diem**************////////////
void thaotacmenuDIEM(PTRDIEM &listDIEM,list_MH &listMH,list_LOP &listLOP)
{
	setcolor(16,15);
	system("cls");
	...
	gotoxy(25,3);cout<<"                              ";
	VeEditText(30,5,"    Nhap Ma Lop    ");
    VeEditText(30,7,"  Nhap Ma Mon Hoc  ");
    gotoxy(5,36);                                  
NHAPMALOP:
	NhapMaLOpDiem(10,18);
    ...
    switch(c)
	 {
	  	case KEY_F5:           
	        ...
	        gotoxy(x+30,y+1);
	        cout<<"  "<<menu[2];
	        HienThiGoiYMH(listMH,16,15,x-18,5+y);
	        setcolor(16,16);
			for ( int i = 0 ; i <  x+5 ; i++)
			{
	  	...
	  	case KEY_ENTER:
			...
			{
				ThongBao("Chua Nhap Ma Lop!",16,28,25,15);
				goto NHAPMALOP;
	  		}
	  		vitrilop=KiemTra_Lop(listLOP,malop);
	  		if(vitrilop==-1)
			  {
	  			ThongBao("Khong Tim Thay Ma Lop !",16,28,30,15);
	  			goto NHAPMALOP;
	  		  }
		    break;
		case KEY_UP:
		...
		case KEY_DOWN:
			while(strlen(malop)==0)
			{
				ThongBao("Chua Nhap Ma Lop!",16,28,30,15);
				goto NHAPMALOP;
	  		}
	  		vitrilop=KiemTra_Lop(listLOP,malop);
	  		if(vitrilop==-1)
    ...
    switch(c)
	 {
	  	...
	  	case KEY_ENTER:
	  		while(strlen(mamh)==0)
			{
				ThongBao("Chua Nhap Ma Mon Hoc!",16,28,25,15);
				goto NHAPMAMH;
	  		}
	  		vitrimh=KiemtraMAMH(listMH,mamh);
	  		if(vitrimh==-1)
			  {
	  			ThongBao("Khong Tim Thay Ma MH !",16,28,25,15);
	  			goto NHAPMAMH;
	  		  }
		    break;
	...
	for(PTRDIEM q =listDIEM;q!=NULL;q=q->next)
		 {
		   for(int i=0;i<soluong;i++)
		   {
	   	  	if(strcmp(sv[i].MASV,q->diem.MASV)==0&&strcmp(mamh,q->diem.MAMH)==0&&q->diem.LAN==1)
			{
				if(q->diem.DIEMSV ==10)
				{
				...
				{
				    snprintf(sothuc, sizeof(sothuc), "%f", q->diem.DIEMSV);
				}
		        strcpy(diemlan1[i],sothuc);
		        break;
		    }
	   	  	if(strcmp(sv[i].MASV,q->diem.MASV)==0&&strcmp(mamh,q->diem.MAMH)==0&&q->diem.LAN==2)
			{
				if(q->diem.DIEMSV ==10)
				{
					snprintf(sothuc, sizeof(sothuc), "%d", (int)(q->diem.DIEMSV));
...
XUATDS:	
	...
	gotoxy(3,6);  cout<<" MON HOC  : "<<listMH.mh[vitrimh].TENMH ;
	setcolor(16,15);
	BangDiem(15,4,7);
	...
	if(sluongSV ==0)
	{
		ThongBao("Chua Co Sinh Vien Trong Lop Nay !",16,28,25,38);
	}
	...
	NHAPMASV:   
			...
			{
				ThongBao("Ma SV khong ton tai !!",16,28,25,38);
	        	goto NHAPMASV;
	        }
	        if(Xoa_ALL_DIEM_MH(listDIEM,masv,mamh)!=0){
	        	ThongBao("Xoa Diem Thanh Cong !!",16,28,25,38);
	        	goto LAYDIEM;
	        }else{
	        	ThongBao("Sinh Vien Khong Co Diem Mon Nay !!",16,28,25,38);
	        	goto XUATDS;
	        }
		    break;    
		...
		case KEY_ESC:
			...
			if(kq==1)
			{
			   kq =CapNhatDanhSachDiem(listDIEM,mamh,soluong,diemlan1,diemlan2,sv,tenfile);
			   if(kq==1)
			       ThongBao("Luu File Thanh Cong !!",16,28,25,38);
			     else
			      ThongBao("Luu File That Bai !!",16,28,25,38);
...
void DiemTongKetLop(PTRDIEM &listDIEM,list_MH &listMH,list_LOP &listLOP)
{
	setcolor(16,15);
	system("cls");
	char chucnangdiem,ch;
...
NHAPMALOP:
	...
	char TongKetDiem[listMH.n][soluong][4];
	for(int i=0;i<listMH.n;i++)
	{
		tcmonhoc[i]=0;
	...
	for(PTRDIEM p=listDIEM ; p!=NULL;p=p->next)
	{
		vitriMH =KiemtraMAMH(listMH,p->diem.MAMH);
		if(vitriMH !=-1)
		{
		  for(int i = 0; i < soluong ; i++)
		  {
			char diem[4];
			if(strcmp(sv[i].MASV,p->diem.MASV)==0)
			{
				tcmonhoc[vitriMH] = (listMH.mh[vitriMH].STCLT+listMH.mh[vitriMH].STCTH);
				if((p->diem.DIEMSV) >= atof(TongKetDiem[vitriMH][i]))
				{
				  ...
				  {
				     snprintf(diem, sizeof(diem), "%f", p->diem.DIEMSV);
			      }
...
XUATDS :
	 ...
	 for(int i= 0 + trangSV*15 ; i < 15 + trangSV*15 && i< soluong ; i++)
	 {
		...
		for (int j = 0; j < 3; j ++)
		{
		   ...
		   if (vitriMH < listMH.n&&TongKetDiem[vitriMH][i][0] != '\0')
			{
	 ...
	 if(check==1)
	 {
	   ...
	   for(int i= 0 + trangSV*15 ; i < 15 + trangSV*15 && i< soluong ; i++)
	   {
	   	 if(tongsotc[i] > 0)
	   	 {
	   	 	...
	   	 	gotoxy ( x+2 , (y+2) +dong) ;
	   	 	char diemtb[4];
			snprintf(diemtb, sizeof(diemtb), "%f", (diemMH[i])/(tongsotc[i]));
		    cout<<diemtb;
	   	 }
	 	 dong++;
	   }
	 ...
	 switch(key)
	 {
		...
		case KEY_F1:
			...
			HienThiGoiYMH(listMH,16,15,x-18,18+y);
	        setcolor(16,16);
			for ( int i = 0 ; i <  x+5 ; i++)
			{
				for ( int j = 0 ; j < y+60 ; j++ )
...
void TaoMenuDiem(PTRDIEM &listDIEM,list_MH &listMH,list_LOP &listLOP){
	char key;
	int chon=-1;
		...
		switch(key)
		{
			...
			case KEY_DOWN:
			 ...
			 case KEY_ENTER:
			 	if(chon==0)
			 	{
			 		thaotacmenuDIEM(listDIEM,listMH,listLOP);
			 	}
				else
				{
					DiemTongKetLop(listDIEM,listMH,listLOP);
				}
			 	return;
			    break;   
		}
